        ...
        try (JerseyDockerHttpClient httpClient = new JerseyDockerHttpClient.Builder()
            ...
            if (!Thread.currentThread().isInterrupted()) {
                if (0 != exitCode) {
                    ...
                    // on thread interruption: Do not expect (nor care about) any result or container log
                ...
                // If the container exited abnormally or a response with is-internal-error=true was returned,
                // print the container log even if the current error logging level is not set to INFO,
                // by temporarily changing the logging level to INFO
                Level originalLevel = null;
                final String packageName = this.getClass().getPackageName();
                try {
                    originalLevel = LogbackHelper.getEffectiveLogLevel(packageName);
                    final Level changeToLevel = Level.INFO;
                    ResponseLive responseLive = new ResponseLive(responseResource);
                    boolean gradingFailed =
                        0 != exitCode || responseLive.getProformaVersion().getResponseHelper().hasInternalError(responseResource);
                    if (gradingFailed && originalLevel.toInt() > changeToLevel.toInt()) {
                        LogbackHelper.setLogLevel(packageName, changeToLevel);
                        log.error("Graiding failed, printing container log");
                    }
                } catch (Exception e) {
                    log.error(e.getMessage());
                    log.error(DebugUtils.getStackTrace(e));
                ...
                // If the logging level changed temporarily, set it back the original value
                try {
                    if (null != originalLevel && !LogbackHelper.getEffectiveLogLevel(packageName).equals(originalLevel)) {
                        LogbackHelper.setLogLevel(packageName, originalLevel);
                    }
                } catch (Exception e) {
                    log.error(e.getMessage());
                    log.error(DebugUtils.getStackTrace(e));
